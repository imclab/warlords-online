...
public class GameSprite extends Sprite {
	...
	// && (Math.abs(pTouchAreaLocalX) + Math.abs(pTouchAreaLocalY) >
	// ACTION_MOVE_THRESHOLD)) {
	// // element is dragged
	// mIsGrabbed = true;
	// }
	// if (mIsSelected && mIsGrabbed) {
	// mInputManager.updateOrderLine(this, pSceneTouchEvent.getX(),
	...
	// if (Math.abs(pSceneTouchEvent.getX() - getX()) +
	// Math.abs(pSceneTouchEvent.getY() - getY()) > VALID_ORDER_THRESHOLD) {
	// // give order to unit
	// mInputManager.giveOrderToUnit(pSceneTouchEvent.getX(),
	// pSceneTouchEvent.getY());
	// }
	// } else if (wasSelected
	// && Math.abs(pSceneTouchEvent.getX() - getX()) +
	// Math.abs(pSceneTouchEvent.getY() - getY()) < VALID_ORDER_THRESHOLD) {
	...
	// specialSprite = GraphicElementFactory.createSprite(35, -50,
	// "muzzle_flash.png", getVertexBufferObjectManager());
