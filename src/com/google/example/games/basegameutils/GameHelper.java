...
public class GameHelper implements GooglePlayServicesClient.ConnectionCallbacks,
	...
	public void setup(GameHelperListener listener, int clientsToUse, String... additionalScopes) {
		...
		if (0 != (clientsToUse & CLIENT_GAMES)) {
			...
			mGamesClient = new GamesClient.Builder(getContext(), this, this)
			        .setGravityForPopups(Gravity.TOP | Gravity.CENTER_HORIZONTAL).setScopes(mScopes).create();
		}
	...
	public void showAlert(String message) {
		(new AlertDialog.Builder(getContext())).setMessage(message).setNeutralButton(android.R.string.ok, null)
		        .create().show();
	}
	...
	public String getInvitationId() {
		if (!checkState(TYPE_DEVELOPER_ERROR, "getInvitationId", "Invitation ID is only available when connected "
	...
	public void onActivityResult(int requestCode, int responseCode, Intent intent) {
		debugLog("onActivityResult: req=" + (requestCode == RC_RESOLVE ? "RC_RESOLVE" : String.valueOf(requestCode))
		        + ", resp=" + activityResponseCodeToString(responseCode));
	...
	void connectNextClient() {
		...
		debugLog("connectNextClient: requested clients: " + mRequestedClients + ", connected clients: "
		        + mConnectedClients);
		...
		if (mGamesClient != null && (0 != (pendingClients & CLIENT_GAMES))) {
			...
			throw new AssertionError("Not all clients connected, yet no one is next. R=" + mRequestedClients + ", C="
			        + mConnectedClients);
		}
	...
	void succeedSignIn() {
		checkState(TYPE_GAMEHELPER_BUG, "succeedSignIn", "succeedSignIn should only "
		        + "get called in the connecting or connected state. Proceeding anyway.", STATE_CONNECTING,
		        STATE_CONNECTED);
	...
	Dialog makeSimpleDialog(String text) {
		return (new AlertDialog.Builder(getContext())).setMessage(text).setNeutralButton(android.R.string.ok, null)
		        .create();
	}
	...
	String getSHA1CertFingerprint() {
		try {
			Signature[] sigs = getContext().getPackageManager().getPackageInfo(getContext().getPackageName(),
